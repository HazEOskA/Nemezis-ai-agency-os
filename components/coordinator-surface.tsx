...
function Dashboard({state, onOpenModule}: {state: DemoState; onOpenModule: (module: CoordinatorModule) => void}) {
  ...
  return <>
    <Header title="Good morning, Anna" subtitle="Your coordination desk · Monday, June 30, 2026" action="Open today planning" onAction={() => onOpenModule('planning')} />
    ...
    <section className="branch-two-column"><div className="branch-panel"><div className="branch-panel-heading"><div><span className="branch-kicker">LIVE PLANNING</span><h2>Next actions for your workers</h2></div><span className="branch-count">{today.length}</span></div><div className="branch-list">{today.map((shift) => <button className="branch-list-row" type="button" key={shift.id} onClick={() => onOpenModule('planning')}><Dot tone={shift.status === 'attention' ? 'orange' : 'green'} /><span><strong>{shift.workerName}</strong><small>{shift.client} · {shift.start}–{shift.end} · {shift.status}</small></span><b>›</b></button>)}</div></div><div className="branch-panel"><div className="branch-panel-heading"><div><span className="branch-kicker">ESCALATIONS</span><h2>Needs a human decision</h2></div><span className="branch-count branch-count-red">{state.case?.status === 'open' ? 1 : 0}</span></div><button className="branch-callout branch-callout-red" type="button" onClick={() => onOpenModule('cases')}><Dot tone={state.case?.status === 'open' ? 'red' : 'green'} /><span><strong>{state.case?.status === 'open' ? 'Transport problem · Mila Ionescu' : 'No urgent Buddy case'}</strong><small>{state.case?.status === 'open' ? 'FreshLogistics · pickup 05:15' : 'Open the case desk to review the queue.'}</small></span></button><button className="branch-secondary-button" type="button" onClick={() => onOpenModule('cases')}>Open case queue</button></div></section>
  </>;
}

function Planning({state, onAcknowledge, onSendReminder, pending}: {state: DemoState; onAcknowledge: (shiftId: string) => void | Promise<void>; onSendReminder: () => void | Promise<void>; pending: PendingAction}) {
  return <><Header title="Today Planning" subtitle="Review assignments, transport and worker readiness before the shift starts" action="Send planning reminder" onAction={onSendReminder} /><div className="branch-planning-list">{state.shared.planning.map((shift) => <div className="branch-planning-card" key={shift.id}><div className="branch-planning-date"><strong>{shift.dateLabel}</strong><span>{shift.start}–{shift.end}</span></div><div className="branch-planning-main"><h2>{shift.workerName}</h2><p>{shift.client} · {shift.location}</p><small>{shift.transport} · Coordinator: {shift.coordinator}</small></div><div className="branch-planning-status"><span className={`branch-status branch-status-${shift.status}`}><Dot tone={shift.status === 'attention' ? 'orange' : shift.status === 'complete' ? 'green' : 'blue'} />{shift.status}</span>{shift.acknowledged ? <small className="branch-success-text">Worker acknowledged</small> : <button className="branch-secondary-button" type="button" onClick={() => onAcknowledge(shift.id)} disabled={pending === 'planning'}>Mark reviewed</button>}</div></div>)}</div></>;
}

function Messenger({state, onSendMessage, onBroadcast, pending}: {state: DemoState; onSendMessage: (input: {body: string; actor: 'coordinator'; recipient: string; recipientRole: 'worker'}) => void | Promise<void>; onBroadcast: () => void | Promise<void>; pending: PendingAction}) {
...
export default function CoordinatorSurface({state, t, activeModule, onReset, onModuleChange, onResolve, onAcknowledge, onSendMessage, actionPending}: {state: DemoState | null; t: Translator; activeModule: CoordinatorModule; onReset: () => void | Promise<void>; onModuleChange: (module: CoordinatorModule) => void; onResolve: () => void | Promise<void>; onAcknowledge: (shiftId: string) => void | Promise<void>; onSendMessage: (input: {body: string; actor: 'coordinator'; recipient: string; recipientRole: 'worker'}) => void | Promise<void>; actionPending: PendingAction}) {
  ...
  const sendReminder = () => onSendMessage({body: 'Planning reminder: please review your next assignment in Worker Preview.', actor: 'coordinator', recipient: 'Assigned worker group', recipientRole: 'worker'});
  const sendBroadcast = () => onSendMessage({body: 'Important coordination update: please check your planning and contact the desk if anything is unclear.', actor: 'coordinator', recipient: 'Assigned worker group', recipientRole: 'worker'});
  const content = activeModule === 'dashboard' ? <Dashboard state={state} onOpenModule={onModuleChange} /> : activeModule === 'planning' ? <Planning state={state} onAcknowledge={onAcknowledge} onSendReminder={sendReminder} pending={actionPending} /> : activeModule === 'messenger' ? <Messenger state={state} onSendMessage={onSendMessage} onBroadcast={sendBroadcast} pending={actionPending} /> : activeModule === 'workers' ? <Workers state={state} onOpenModule={onModuleChange} /> : activeModule === 'cases' ? <Cases state={state} onResolve={onResolve} pending={actionPending} /> : <Numbers state={state} />;
  return <div className="branch-surface"><div className="branch-toolbar"><div><span className="branch-kicker">NEMEZISAI / COORDINATOR</span><strong>{navigation.find((item) => item.id === activeModule)?.label}</strong></div><button className="branch-secondary-button" type="button" onClick={onReset} disabled={actionPending === 'reset'}>↻ Reset demo</button></div>{content}</div>;
}
